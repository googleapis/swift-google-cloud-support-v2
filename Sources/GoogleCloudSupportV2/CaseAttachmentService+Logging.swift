...
extension Clients {
  final class CaseAttachmentServiceLogging: CaseAttachmentServiceStub {
    let inner: any CaseAttachmentServiceStub
    let logger: Logger

    public init(_ inner: any CaseAttachmentServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-support-v2"
      logger[metadataKey: "gcp.client.service"] = "cloudsupport"
      logger[metadataKey: "gcp.experimental.swift.client"] = "CaseAttachmentService"
      self.inner = inner
      self.logger = logger
    }
