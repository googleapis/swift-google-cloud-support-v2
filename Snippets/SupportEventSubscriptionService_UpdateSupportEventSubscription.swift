...
func sample(
  ...
  supportEventSubscriptionId: String
) async throws {
  let response = try await client.updateSupportEventSubscription(
    request: UpdateSupportEventSubscriptionRequest()
      .with {
        $0.supportEventSubscription = SupportEventSubscription().with {
          $0.name =
...
struct SnippetRunner {
  static func main() async throws {
    do {
      ...
      try await sample(
        client: client, organizationId: "[placeholder]", supportEventSubscriptionId: "[placeholder]"
      )
    } catch {
      print("Error: \(error)")
    }
